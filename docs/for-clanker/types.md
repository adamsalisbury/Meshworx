...
does, at `MeshHub.cs:1171`). Trivially constructible in tests.
...
- **Only guaranteed valid for the duration of the call** — copy it if it must outlive the invocation.
  (In the current implementation the hub already copies it out of the inbound frame, `MeshHub.cs:1170`, so
...
- `ValueTask<bool>` — a synchronous decision (`ValueTask.FromResult(...)`) takes an explicitly
  allocation-free fast path in the hub (`MeshHub.cs:1517-1522`) and is the common case; anything else,
...
A `sealed record` with `required` init-only properties — construct with an object initialiser (the hub
does, at `MeshHub.cs:1490`). Trivially constructible in tests.
