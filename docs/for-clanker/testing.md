...
| `TopicSubscriptionTrieTests.cs` | 217 | **New for issue #37.** `TopicSubscriptionTrie` in isolation: exact-literal match, single-segment (`+`) and multi-segment (`#`) wildcard matching (`Theory`-driven), overlapping subscriptions deduplicated to one result per subscriber, no-subscribers returns empty, unsubscribe (including of an unknown pattern, and pruning the now-empty trie nodes a last subscriber on a branch leaves behind), malformed patterns (`#` not in final position, and the `Theory`-driven malformed-pattern cases) throwing `ArgumentException` rather than being accepted, a concrete topic rejecting a wildcard segment, and the 128-segment cap on both `Subscribe` and `Match` throwing rather than overflowing the stack. **Updated by commit `fb2f9a0`** to declare `Match`'s result as `IReadOnlySet<Guid>` (perf commit `ec1eab9` changed the return type from a copied `IReadOnlyList<Guid>` to the trie's own backing set) |
| `TopicPubSubClientTests.cs` | 102 | **New, commit `01b5765`.** The client-side half of the two gaps commit `fb2f9a0` fixed (see [known-issues.md](known-issues.md) KI-61): `SubscribeAsync`/`PublishAsync` reject a malformed pattern/topic (empty, misplaced `#`, a wildcard in a concrete topic) with `ArgumentException` *before* ever calling the mocked transport (`SubscribeAsync_MalformedPattern_NeverSendsAFrame` asserts the transport saw only the registration send); and `SubscribeAsync`/`UnsubscribeAsync`/`PublishAsync` each throw `NotSupportedException` one version below `Protocol.TopicPubSubMinVersion` and succeed exactly at it |
| `MeshIntegrationTests.cs` | 1024 (row previously read 540 — already stale by a wide margin pre-dating this pass, per the standing note against fully re-deriving `testing.md`'s line counts every pass) | Hub + real clients over `InMemoryTransport`, end-to-end, plus **one mutual-TLS run over real sockets**, **non-member/unauthorised group paths**, **end-to-end header delivery across mixed negotiated versions, PR #74**, and — **new for issue #37** — four topic pub/sub end-to-end tests: a wildcard-pattern subscriber receiving a published message, a publish with no matching subscribers being a genuine no-op, an unsubscribed client no longer receiving further topic messages, and subscriptions being cleared on disconnect |
...
| `Transport/Unix/TempSocketPath.cs` | 13 | **New in PR #81 (issue #20).** Helper only: generates a short `{Guid:N}.sock` path under the temp directory, kept short deliberately to stay under the platform's `sun_path` length limit |
...
- **The WebSocket transport's tests (PR #78, issue #18) follow the TCP transport's conventions
  ...
  - **`WebSocketMeshIntegrationTests.cs` mirrors `MeshIntegrationTests.cs`'s mutual-TLS end-to-end run**
    ...
    demonstrates.
- **The Unix domain socket transport's tests (PR #81, issue #20) follow the
...
- **The QUIC transport's tests (PR #82, issue #21) are documented no-ops
