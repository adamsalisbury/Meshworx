...
2. **Message** (owned by hub/client). The **first payload byte is the opcode** (`MessageType`); the rest
   is opcode-specific. Empty frames (length 0) are ignored, not decoded (`MeshHub.cs:627`,
   `MeshClient.cs:511`).
...
Hub-side validation order (`MeshHub.cs:510-602`), each failure sends the error (if applicable) and drops
the connection:
1. Frame ≥ **2** bytes and opcode `0x04` — else drop silently (no error frame) (`:510-515`).
2. `version == 3` — else `Error(UnsupportedProtocolVersion)` (`:517-523`). **This is checked before the
   length checks below**, so a 2-byte frame carrying the wrong version still gets an error reply.
3. Frame ≥ 4 bytes, i.e. long enough to carry the name length — else drop silently (`:525-529`).
4. `nameLen != 0` **and** frame ≥ `4 + nameLen` — else drop silently (`:531-538`). A declared length of
...
5. Decode the name from bytes `[4, 4+nameLen)`; `clientName.Length ≤ 256` **chars** — else
   `Error(ClientNameTooLong)` (`:542-548`).
6. **Authentication**, only when an authenticator was configured (`:550-571`). Two parts, in order: an
   at-capacity **early-out** — already-claimed slots `>= maxClients` → `Error(HubAtCapacity)` without the
   callback running (`:557-561`) — then the callback itself, given the name and credential
   (`:563-570`). Refusal, throw, cancellation or timeout → `Error(AuthenticationFailed)`.
7. **Capacity claim** (`:578-582`): one atomic compare-and-swap takes a client slot if and only if fewer
...
8. Name not already claimed (`_clientNames.TryAdd`) — else `Error(DuplicateClientName)` (`:586-591`).
...
dropped without an error frame (`MeshHub.cs:498-506`).
...
broadcast from a direct message (`MeshHub.BroadcastMessage` builds a `0x03` frame, `MeshHub.cs:1086`):
```
...
outbound `DeliverGroupMessage` rather than re-encoding the decoded string (`MeshHub.cs:662-668`,
`:1215-1220`).
...
(`MeshHub.cs:633-705`, `MeshClient.cs:513-643`). A frame that is too short for its opcode, or carries an
...
name length running past the payload drops the connection with **no error frame** (`MeshHub.cs:525-538`).
