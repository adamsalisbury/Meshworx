...
2. **Message** (owned by hub/client). The **first payload byte is the opcode** (`MessageType`); the rest
   is opcode-specific. Empty frames (length 0) are ignored, not decoded (`MeshHub.cs:920`,
   `MeshClient.cs:546`).
...
Hub-side validation order (`MeshHub.cs:803-895`), each failure sends the error (if applicable) and drops
the connection:
1. Frame ≥ **2** bytes and opcode `0x04` — else drop silently (no error frame) (`:803-808`).
2. `version == 3` — else `Error(UnsupportedProtocolVersion)` (`:810-816`). **This is checked before the
   length checks below**, so a 2-byte frame carrying the wrong version still gets an error reply.
3. Frame ≥ 4 bytes, i.e. long enough to carry the name length — else drop silently (`:818-822`).
4. `nameLen != 0` **and** frame ≥ `4 + nameLen` — else drop silently (`:824-831`). A declared length of
...
5. Decode the name from bytes `[4, 4+nameLen)`; `clientName.Length ≤ 256` **chars** — else
   `Error(ClientNameTooLong)` (`:835-841`).
6. **Authentication**, only when an authenticator was configured (`:843-864`). Two parts, in order: an
   at-capacity **early-out** — already-claimed slots `>= maxClients` → `Error(HubAtCapacity)` without the
   callback running (`:850-854`) — then the callback itself, given the name and credential
   (`:856-863`). Refusal, throw, cancellation or timeout → `Error(AuthenticationFailed)`.
7. **Capacity claim** (`:871-875`): one atomic compare-and-swap takes a client slot if and only if fewer
...
8. Name not already claimed (`_clientNames.TryAdd`) — else `Error(DuplicateClientName)` (`:879-884`).
...
dropped without an error frame (`MeshHub.cs:791-799`).
...
broadcast from a direct message (`MeshHub.BroadcastMessage` builds a `0x03` frame, `MeshHub.cs:1390`):
```
...
outbound `DeliverGroupMessage` rather than re-encoding the decoded string (`MeshHub.cs:958-964`,
`:1711-1716`).

**`GroupJoinRefused` echoes the same bytes, and that is load-bearing rather than tidy.** The hub copies
the inbound `JoinGroup` name bytes and replies with exactly those (`RefuseGroupJoin`, `MeshHub.cs:1574`,
copy at `:1446`, echo at `:1584-1586`). Re-encoding the *decoded* string is not size-preserving: every
...
joined the target group (`MeshHub.cs:1683`, `:1693-1699`). There is no error frame for this — a correct
...
(`MeshHub.cs:926-1002`, `MeshClient.cs:714-835`). A frame that is too short for its opcode, or carries an
...
(`MeshHub.cs:1427`) — would itself fall through and be ignored.

The **registration frame follows the same rule**: a truncated frame, a zero name length, or a declared
name length running past the payload drops the connection with **no error frame** (`MeshHub.cs:818-831`).
